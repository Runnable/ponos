...
Worker.prototype.run = function () {
  ...
  return Promise.resolve().bind(this)
    .then(function checkAttemptCounter () {
      ...
      // if we've never set MAX_RETRIES (or set to 0), run the task forever!
      if (!process.env.WORKER_MAX_RETRIES) { return; }
      var maxRetries = process.env.WORKER_MAX_RETRIES;
    ...
    .then(function successDone (result) {
      log.info(merge(jobAndQueueData, { result: result }), 'Job complete');
      return this.done();
    })
