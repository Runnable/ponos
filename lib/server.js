...
class Server {
  ...
  subscribe (exchangeInfo) {
    this.log.trace(exchangeInfo, 'subscribing to exchange')

    var self = this
    function wrapper (job, done) {
      const opts = {}

      defaults(opts, {
        queue: 'exchange',
        job: job,
        task: exchangeInfo.handler,
        done: done,
        log: self.log,
        server: self,
        errorCat: self.errorCat
      })

      Worker.create(opts)
    }

    return this._rabbitmq.subscribeToTopicExchange(
      exchangeInfo.exchange,
      exchangeInfo.routingKey,
      wrapper
    )
  }
  ...
  _subscribeAll () {
    ...
    return Promise.resolve(this.opts.exchanges)
      .map((info) => {
